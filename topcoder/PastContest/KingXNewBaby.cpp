...
public:
   ...
   string isValid( string name ) {
      ...
      vector<char> vowel;
      for(int i = 0; i < sz(name); ++i) if(is_vowel(name[i])) vowel.push_back(name[i]);
      if(sz(vowel) != 2)  return "NO";
      return vowel[0] == vowel[1] ? "YES": "NO";
   }
};
...
namespace moj_harness {
   ...
   int run_test_case( int casenum__ ) {
      ...
      case 0: {
         ...
         return verify_case( casenum__, starttime__, expected__, KingXNewBaby().isValid( name ) );
      }
      case 1: {
