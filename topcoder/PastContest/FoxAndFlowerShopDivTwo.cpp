         ...
         for(int j = 0; j < M; ++j) {
            for(int ii = i; ii < N; ++ii) {
               for(int jj = j; jj < M; ++jj) {
                  if(r >= i && r <= ii && c >= j && c <= jj) continue;
                  int cnt = 0;
                  for(int k = i; k <= ii; ++k)
                     for(int v = j; v <= jj; ++v)
                        if(flowers[k][v] == 'F') cnt++;
                  res = max(res, cnt);
               }
            }
         }
...
namespace moj_harness {
   ...
   int run_test_case( int casenum__ ) {
      ...
      case 0: {
         ...
         return verify_case( casenum__, starttime__, expected__, FoxAndFlowerShopDivTwo().theMaxFlowers( vector <string>( flowers, flowers + ( sizeof flowers / sizeof(string) ) ), r, c ) );
      }
      case 1: {
         string flowers[]          = {"F..",  "...",  "..."};
         int r                     = 0;
