...
public:
   ...
   bool allBlack(string s) {
      for(int i = 0; i < sz(s); ++i)
         if(s[i]=='W') return false;
      return true;
   }
   
   bool allWhite(string s) {
      for(int i = 0; i < sz(s); ++i)
   ...
   int countBlack( string ballSequence, int repetitions ) {
      int cnt = 0;
      if(allBlack(ballSequence)) return 1;
      if(allWhite(ballSequence)) return 0;
      string s = "";
...
namespace moj_harness {
   ...
   int run_test_case( int casenum__ ) {
      ...
      case 4: {
         string ballSequence       = "BWBWWWWWWWWBBBWWWBBWWWWBBBBWBBBBBWWW";
